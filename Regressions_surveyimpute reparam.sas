...
proc means data=model_dat_nomiss;
  by d_female;
  where feed_type_3mos=1 and current_smoke_grp=1;
  var waz_change;
  title "feed_type_3mos=1 and current_smoke_grp=1";run;
proc means data=model_dat_nomiss;
  by d_female;
  where feed_type_3mos=1 and current_smoke_grp=2;
  var waz_change;
  title "feed_type_3mos=1 and current_smoke_grp=2";run;
proc means data=model_dat_nomiss;
  by d_female;
  where feed_type_3mos=1 and current_smoke_grp=3;
  var waz_change;
  title "feed_type_3mos=1 and current_smoke_grp=3";run;
run;
proc means data=model_dat_nomiss;
  by d_female;
  where feed_type_3mos=2 and current_smoke_grp=1;
  var waz_change;
  title "feed_type_3mos=2 and current_smoke_grp=1";run;
proc means data=model_dat_nomiss;
  by d_female;
  where feed_type_3mos=2 and current_smoke_grp=2;
  var waz_change;
  title "feed_type_3mos=2 and current_smoke_grp=2";run;
proc means data=model_dat_nomiss;
  by d_female;
  where feed_type_3mos=2 and current_smoke_grp=3;
  var waz_change;
  title "feed_type_3mos=2 and current_smoke_grp=3";run;
run;
proc means data=model_dat_nomiss;
  by d_female;
  where feed_type_3mos=3 and current_smoke_grp=1;
  var waz_change;
  title "feed_type_3mos=3 and current_smoke_grp=1";run;
proc means data=model_dat_nomiss;
  by d_female;
  where feed_type_3mos=3 and current_smoke_grp=2;
  var waz_change;
  title "feed_type_3mos=3 and current_smoke_grp=2";run;
proc means data=model_dat_nomiss;
  by d_female;
  where feed_type_3mos=3 and current_smoke_grp=3;
  var waz_change;
  title "feed_type_3mos=3 and current_smoke_grp=3";run;
run;

proc freq data=model_dat_nomiss;
tables feed_type_3mos*current_smoke_grp /missing;
run;
*----------------------*;
*/
...
run;
  ...
  title "*----------* Interaction terms - add covariates, no main effect *----------* ";
	PROC SURVEYREG data=model_dat_nomiss;	
	class feed_type_3mos  (ref=first)
		  current_smoke_grp (ref=first)
		  pregnancy_smoke_grp (ref=first)
		  parity(ref=first)
		  Married(ref=first)
		  poverty(ref=first)
		  d_OtherUK(ref=first)
	...
	model waz_change= 
		  pregnancy_smoke_grp parity Married poverty d_OtherUK d_nonwhite d_female d_degree ADDAGB00 ADGEST00 
  		  feed_type_3mos*current_smoke_grp /solution;
	strata  pttype2;
	cluster  MCSID sptn00; 
	weight  aovwt2;
...
run;
  ...
  title "*----------* Interaction terms - add covariates, no main effect - by feeding type *----------* ";
    proc sort
	PROC SURVEYREG data=model_dat_nomiss;	
	class feed_type_3mos  (ref=first)
		  current_smoke_grp (ref=first)
