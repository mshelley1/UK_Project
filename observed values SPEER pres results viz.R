...
# Read SAS data set, munge the factor vars
  ...
  dat<-read_sas(path) %>%
       select(c(current_smoke_grp, waz_change, feed_type_3mos, d_female)) %>%
...
# Plot waz_change for smoke grps and feeding type and sex.     
  ggplot(dat, aes(x = current_smoke_grp, y = waz_change)) +
    geom_boxplot() +
    stat_summary(fun = mean, geom = "point", shape = 18, size = 3, color = "red") +
    facet_wrap(~feed_type_3mos*d_female, nrow=3, ncol=2) +
    labs(title = "Change in baby's weight by mother's smokng, feeding method, and baby's sex ",
         x = "Mother's smoking since baby's birth",
         y = "Change in baby's weight since birth (weight for age Z-score)") +
    scale_x_discrete(labels=scales::wrap_format(width=10))
