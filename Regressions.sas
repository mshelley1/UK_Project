...
/*
  ...
  *class pttype2 pregnancy_smoke_grp current_smoke_grp feed_type_3mos health_probs parity treat_now_depression DSA_grp Married d_illness d_nonwhite d_female d_degree d_seeFriends;
  *fcs discrim(classeffects=include);
  VAR pttype2  health_probs parity treat_now_depression DSA_grp Married d_illness d_nonwhite d_female d_degree d_seeFriends poverty
		ADDAGB00 ADGEST00 ;
*/
...
* Run model on imputed data. Include by to do it separately for each of the imputed data sets. We use surveyreg instead of glm;
  title "Multiple imputation model";
	PROC SURVEYREG data=imputed_dat (obs=3000);	
	class pregnancy_smoke_grp current_smoke_grp feed_type_3mos /*health_probs*/ parity /*treat_now_depression DSA_grp*/ Married poverty d_OtherUK /*d_illness*/ d_nonwhite d_female d_degree /*d_seeFriends*/;
...
run;
proc freq data=imputed_dat; tables feed_type_3mos;run;
...
* Combine output from model results on the imputed data sets;
  ...
  modeleffects pregnancy_smoke_grp current_smoke_grp feed_type_3mos /*health_probs*/ parity /*treat_now_depression DSA_grp*/ Married poverty d_OtherUK /*d_illness*/ d_nonwhite d_female d_degree /*d_seeFriends*/
		ADDAGB00 ADGEST00;
run;
