...
* File name version 2 replaces feed_type_3mos with FeedType and FeedType_num;
*-----------------*;
...
*set analysis.analysis_dat_5; *reordered feedtype_3mos; 
set analysis.analysis_dat_6; *refactored feeding type;
	...
	* Make FeedType ordinal/numeric and create EverBreastFed;
		If FeedType="Breast" then FeedType_num =1 ;
		Else if FeedType="Mixed" then FeedType_num=2;
		Else if FeedType="Formula" then FeedType_num=3;

		If FeedType in ("Breast","Mixed") then EverBreastFed=1;
		Else if FeedType="Formula" then EverBreastFed=0;
...
*-------------------------------------------------------------------------------------*;
 ...
 data model_dat;
	retain MCSID pttype2 sptn00 aovwt2 waz_change waz_birth pregnancy_smoke_grp current_smoke_grp FeedType_num EverBreastFed ADDAGB00 ADGEST00 parity 
			 poverty  Married d_nonwhite d_female d_otherUK d_degree;  
set dat1 (keep=MCSID ACNUM00 pttype2 sptn00 aovwt2 waz_change waz_birth pregnancy_smoke_grp current_smoke_grp FeedType_num EverBreastFed ADDAGB00 ADGEST00 parity 
...
* Impute;
  ...
  class pregnancy_smoke_grp current_smoke_grp FeedType_num parity Married poverty d_OtherUK d_nonwhite d_female d_degree;
  strata  pttype2;
...
* Keep only rows that have values for all vars in the models so obs aren't dropped and all models are run on the same set of obs;
  data model_dat_vars;
  set imputed_dat (keep=MCSID ACNUM00 waz_change pregnancy_smoke_grp current_smoke_grp FeedType_num EverBreastFed parity Married poverty d_OtherUK d_nonwhite d_female d_degree ADDAGB00 ADGEST00 pttype2 MCSID sptn00 aovwt2);run;
...
* Run model on imputed data. ;
  ...
  title "*------------* Main Effects Only *-----------------*"; 
	...
	class pregnancy_smoke_grp(ref=last) current_smoke_grp(ref=last) FeedType_num(ref=last) parity(ref=first) Married(ref=first)
		  poverty(ref=first) d_OtherUK(ref=first) d_nonwhite(ref=first) d_female(ref=first) d_degree(ref=first) ;
	model waz_change= pregnancy_smoke_grp current_smoke_grp FeedType_num  parity Married poverty d_OtherUK d_nonwhite d_female d_degree ADDAGB00 ADGEST00 /solution;
	strata  pttype2;
	cluster  MCSID sptn00;
	weight  aovwt2;
...
run;
  ...
  title "*----------* Interaction terms - no covariates; no main effect  *----------* ";
	...
	class  current_smoke_grp (ref=last)
		  FeedType_num  (ref=last) ;
	model waz_change= 
  		  current_smoke_grp*FeedType_num /solution;
	strata  pttype2;
	cluster  MCSID sptn00; 
	weight  aovwt2;
	*ods output ParameterEstimates=a_parm_est;
run;
  ...
  title "*----------* Interaction terms - add covariates, no main effect *----------* ";
	...
	cluster  MCSID sptn00; 
	weight  aovwt2;
	ods output ParameterEstimates = InteractParmEst;
	*output out=outmodel_interaction
			predicted=predicted residual=residuals std=stderr;
	data analysis.interactparmest;
	set interactparmest;
run;


  title "*----------* Ever breast fed - Interaction terms - add covariates, no main effect *----------* ";
	PROC SURVEYREG data=model_dat_nomiss;	
	class current_smoke_grp (ref=last)
		  EverBreastFed  (ref=first)
		  pregnancy_smoke_grp (ref=last)
		  parity(ref=first)
		  Married(ref=first)
		  poverty(ref=first)
	...
	model waz_change= 
		  pregnancy_smoke_grp parity Married poverty d_OtherUK d_nonwhite d_female d_degree ADDAGB00 ADGEST00 
  		  current_smoke_grp*EverBreastFed /solution;
	strata  pttype2;
	cluster  MCSID sptn00; 
	weight  aovwt2;
	ods output ParameterEstimates = EverBreastInteractParmEst;
	*output out=outmodel_interaction
			predicted=predicted residual=residuals std=stderr;
	data analysis.everbreastinteractparmest;
	set everbreastinteractparmest;
run;
...
* Girls v. boys;
  ...
  title "*------------* By sex *----------------*";
	...
	cluster  MCSID sptn00; 
	weight  aovwt2;
	ods output ParameterEstimates = SexEverBreastInteractParmEst;
	*output out=outmodel_interaction
			predicted=predicted residual=residuals std=stderr;
	data analysis.sexeverbreastinteractparmest;
	set sexeverbreastinteractparmest;
run;
title "";
run;
...
* Table 1; * Note: put this in a %include file once it works;
  ...
  set table1_dat (keep=pregnancy_smoke_grp waz_change waz_recent waz_birth ADDAGB00 ADGEST00 current_smoke_grp
						FeedType_num parity poverty Married d_female d_degree ADACAQ00 ADD06E00 COUNTRY
						wt_change);
run;
...
proc contents data=table1_vars position;run;
  ...
  data cat_dat;
  set table1_vars /*(keep=pregnancy_smoke_grp ADD06E00 COUNTRY Married education APLOIL00 FeedType_num poverty d_female) */
  	(keep=pregnancy_smoke_grp current_smoke_grp ADD06E00 COUNTRY ADACAQ00 Married FeedType_num parity  d_female d_degree poverty); 
run;

  * Loop through list of categorical vars, running proc freq for each and creating output data sets;	
	%macro cat_tbl1;
		%local i next_name varn;
		%let varn= current_smoke_grp ADD06E00 COUNTRY ADACAQ00 Married FeedType_num parity  d_female d_degree poverty; /*ADD06E00 COUNTRY Married education APLOIL00 FeedType_num poverty d_female;*/
		%put &varn;
...
*/
	...
	data analysis.OUTMODEL_INTERACTION_2;
